...
- 订阅地址固定为 `http://服务器IP:27695/sub.txt`，包含节点密码和 UUID。服务端会发送禁止缓存的响应头并清理旧订阅文件，但客户端仍可能保留本地缓存；更新后请在客户端手动刷新订阅或删除旧订阅后重新添加。订阅服务通过 HTTP 提供，不适合在不可信网络中直接传输；生产环境应放在 HTTPS 反向代理后，或限制 `27695` 的来源 IP。
