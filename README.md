...
- 每次部署都会生成新的订阅地址，例如 `http://服务器IP:27695/sub-20260914183000-a1b2c3d4.txt`，以绕过客户端缓存；旧订阅文件会被清理。订阅内容包含节点密码和 UUID，服务端也会发送禁止缓存的响应头。更新后请使用脚本最后输出的新地址。订阅服务通过 HTTP 提供，不适合在不可信网络中直接传输；生产环境应放在 HTTPS 反向代理后，或限制 `27695` 的来源 IP。
